const Blogscard = (props) => {
    return ( 
        <div style={{display:'flex', flexDirection:'column', width:'48%', backgroundColor:"white",padding:'1vw', height:'auto', justifyContent:'space-around', borderRadius:'10px', boxShadow:'0px 20px 10px lightgrey'}}>
            <h1 style={{fontSize:'2.3vw'}}>{props.title}</h1>
            <div style={{display:'flex', width:'100%'}}>
                <p style={{fontSize:'1vw'}}><span style={{paddingRight:'1vw'}}>{props.date}</span>  |  <span style={{paddingLeft:'20px'}}>{props.tldr}</span></p>
            </div>
